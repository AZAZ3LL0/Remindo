...
- `v5` — §18 контракт слайса S3: атомы `WizCb` для типа расписания, даты и времени, лимиты напоминания и горизонт мастера в `domain/contracts.py`, публичные `parse_local_date`/`format_local_date`, клавиатуры мастера, ключи текстов, два переименованных ключа `wizard.*`, три новых модуля слайса.
...
### 18.3 Формат даты (`domain/schedules.py`)
...
`daily_times_kb` — единственная клавиатура-тумблер в мастере: повторное нажатие на выбранное время снимает его. Длина списка ограничена §5 двенадцатью элементами.
